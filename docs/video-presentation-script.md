...
> _[Send 'PATCH /api/v1/admin/users/{{targetUserId}}/status']_  
> Look at the response: **HTTP 403 Forbidden**. Message: 'Forbidden: Insufficient permissions'.
>
> Finally, let's log in as a `DRIVER`.  
> _[Click Send on 'Driver Login']_  
> As a driver, I can fetch my ongoing assigned emergency dispatch.  
> _[Send 'GET /api/v1/dispatches/my-active']_  
> Status 200 OK. But if this Driver tries to access Admin user management, they also get blocked with a 403 Forbidden.
>
...
> _[Send 'POST /api/v1/ambulances']_  
...
> _[Send 'GET /api/v1/ambulances?page=1&limit=10']_  
...
> _[Send 'PATCH /api/v1/ambulances/{{ambulanceId}}' with body `{"status": "MAINTENANCE"}`]_  
> Status 200 OK. The status changes to 'MAINTENANCE'. We also log this update in our `AuditLog` table.
>
> **Fourth, DELETE.**  
> _[Send 'DELETE /api/v1/ambulances/{{ambulanceId}}']_  
...
> _[Send 'POST /api/v1/auth/register' with `"email": "not-an-email"`]_  
...
> _[Send request with fake UUID]_  
...
> _[Disable Authorization header and send]_  
...
>
> _[Open the checkoutUrl in the browser]_  
...
> _[Send 'POST /api/v1/payments/stripe/webhook']_  
...
> _[Send 'GET /api/v1/payments/:requestId']_  
